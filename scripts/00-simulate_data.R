...
simulated_shelter_data <-
  tibble(
    sim_location_city = 
      sample(c("Etobicoke",  "North York", "Scarborough","Toronto",  "Vaughan"),
             sample_size,
             replace = TRUE),
    sim_capacity_actual_room = runif(sample_size,10,300),
    sim_occupied_room = sim_capacity_actual_room - round(runif(sample_size,0,10)),
    sim_sector = sample(c("Families","Men Mixed","Adult","Women","Youth") ,
                        sample_size,
                        replace = TRUE))
...
sim_test_2 <- sim_test_1_max <= 300 && sim_test_1_min >= 10
sim_test_2
...
# Test 6 - Verify the number of occupied room  is less than the number of actual room in shelter
sim_test_6 <- simulated_shelter_data$sim_occupied_room <=  simulated_shelter_data$sim_capacity_actual_room
